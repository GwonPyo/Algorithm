# My Solution 1(오답, 32개중 6개 시간 초과)
    ...
    for record in records:                                              # 모든 기록을 탐색한다.
        ...
        if command == 'Enter':
            ...
            if id not in names.keys() or names[id] == name:             # 만약 이전에 들어온 적이 없거나 이전과 동일한 이름으로 들어왔다면
                names[id] = name
                result.append([id, f'{names[id]}님이 들어왔습니다.'])     
                continue
            ...
            result.append([id, f'{names[id]}님이 나갔습니다.'])          
...
def solution(records):
    ...
    for record in records:                                      # 모든 기록을 탐색한다.
        ...
        command = record[0]
        if command == 'Enter':  
            id, name = record[1], record[2]                     # 해당 기록의 id와 이름을 받아온다.
            
            if id not in names.keys() or names[id] != name:     # 이전에 입장한 적이 없거나 이전 이름이랑 다른 이름으로 들어온 경우
                names[id] = name            
            ...
            id = record[1]                                      # 해당 기록의 id를 받아온다.
            result.append([id, '님이 나갔습니다.'])            
        
        else:
            id, name = record[1], record[2]                     # 해당 기록의 id와 이름을 받아온다.
            names[id] = name                                    # 이름을 변경한다.
            
    answer = [names[id]+str for id, str in result]
